...
class SearchPolicy(tf_policy.Base):
	def __init__(self, agent, rb_vec, self._pdist, open_loop=False):
		self._agent = agent
	...
	def _build_graph(self):
		g = nx.DiGraph()
		self._pdist_combined = np.max(self._pdist, axis=0)
		for i, s_i in enumerate(self._rb_vec):
			for j, s_j in enumerate(self._rb_vec):
				length = self._pdist_combined[i, j]
	...
	def _get_path(self, time_step):
		...
		rb_to_goal = agent._get_pairwise_dist(self._rb_vec,
												ts.observation['goal'],
												aggregate='min',
												masked=True).numpy().flatten()
